package com.hjq.shape.view.swipeMenu;
...
/**
 * 1.((SwipeMenuLayout) holder.itemView).setIos(false).setLeftSwipe(position % 2 == 0 ? true : false);//这句话关掉IOS阻塞式交互效果 并依次打开左滑右滑
 * 2.mDatas.remove(pos);
 * mAdapter.notifyItemRemoved(pos);//推荐用这个
 * //如果删除时，不使用mAdapter.notifyItemRemoved(pos)，则删除没有动画效果，
 * //且如果想让侧滑菜单同时关闭，需要同时调用 ((SwipeMenuLayout) holder.itemView).quickClose();
 * //mAdapter.notifyDataSetChanged();
 * 3.item_cst_swipe.xml
 * <p>
 * 【Item侧滑删除菜单】
